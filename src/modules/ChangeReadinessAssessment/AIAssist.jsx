/**
 * CHANGE READINESS ASSESSMENT — AIAssist.jsx
 *
 * FB-1 scaffold derived from _ModuleTemplate. The handleExtract pattern
 * here is documented as a DEVIATION in schema_v0.4.js SCAFFOLDING_NOTES:
 *   "No paste-text-to-extract workflow. Phase 1 input is the wizard,
 *    Phase 2 input is the Excel/CSV import."
 * It exists here only so the AI Assist tab renders during FB-1's
 * scaffold smoke test. FB-5 / FB-6 / FB-8 / FB-13 replace this with
 * the wizard-driven generation, free-chat refinement, and analyze
 * flows. Do not extend this file.
 */
...
export default function AIAssist({ rows, setRows, aiInsights, setAiInsights, initiative, moduleId }) {
    ...
    return (
      <div className="bg-white rounded-xl border border-dashed border-border p-8 text-center">
        ...
        <p className="text-xs text-text-muted max-w-md mx-auto mb-3">
          Add your Anthropic API key in Settings to unlock AI-powered extraction and insights.
        </p>
        <span className="text-xs text-accent font-medium">Settings &rarr; API Key</span>
      </div>
    );
  }

  // --- Helper: call Claude API via centralized client ---
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      // Map AI response fields to your emptyRow() shape:
    ...
    try {
      ...
      const result = await invokeAI(
        enhancePromptWithContext(
          'You are an OCM expert analyst. Provide concise, actionable insights.',
          initiative,
          moduleId
        ),
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-border p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-text-primary">Extract Data from Text</h3>
            <p className="text-[11px] text-text-muted">
              Paste meeting notes, emails, or summaries — AI will structure them into rows
            </p>
          </div>
        </div>
